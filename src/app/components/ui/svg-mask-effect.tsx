...
export const MaskContainer = ({
  ...
  // Handler for React events (if needed elsewhere)
  // Removed unused updateMousePosition function

  // Handler for native events
  const updateMousePositionNative = (e: MouseEvent) => {
  ...
  useEffect(() => {
    if (containerRef.current) {
      containerRef.current.addEventListener("mousemove", updateMousePositionNative);
    }
    return () => {
      if (containerRef.current) {
        containerRef.current.removeEventListener(
          "mousemove",
          updateMousePositionNative,
        );
      }
    };
  }, []);
